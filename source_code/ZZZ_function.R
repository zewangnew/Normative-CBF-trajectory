...
fit_model=function(num){
  ...
  if(i_rnd==0&j_rnd==0)
  {m0<-gamlss(formula=cortex~fp(Age,npoly=i_poly)+Gender,
              sigma.formula =cortex~fp(Age,npoly=j_poly)+Gender,
  ...
  {m0<-gamlss(formula=cortex~fp(Age,npoly=i_poly)+Gender,
              sigma.formula =cortex~fp(Age,npoly=j_poly)+Gender+random(site),
              #nu.formula = feature~1,
              family = GG(mu.link='log',sigma.link = 'log',nu.link = 'identity'),control=con,
              data=M_HC2)
  ...
  } else if(i_rnd==1&j_rnd==0){m0<-gamlss(formula=cortex~fp(Age,npoly=j_poly)+Gender+random(site),
                                          sigma.formula =cortex~fp(Age,npoly=j_poly)+Gender,
                                          #nu.formula = feature~1,
                                          family = GG(mu.link='log',sigma.link = 'log',nu.link = 'identity'),control=con,
                                          data=M_HC2)
  ...
  } else if(i_rnd==1&j_rnd==1){m0<-gamlss(formula=cortex~fp(Age,npoly=i_poly)+Gender + sequence + PLD + random(as.factor(site)),
                                          sigma.formula =cortex~fp(Age,npoly=j_poly)+ Gender + sequence + PLD + random(as.factor(site)),
                                          #nu.formula = feature~1,
                                          family = GG(mu.link='log',sigma.link = 'log',nu.link = 'identity'),control=con,
                                          data=M_HC2)
